...
public class TaskTests
{
    ...
    public void TestSortTask()
    {
        ...
        records.Sort();
        Assert.That(Utilities.CompareList(records, output.List), Is.True);
    }
    ...
    public void TestAccumulatorTask()
    {
        ...
        Assert.That(result, Does.Contain(new Pair<Utf8String, int>(new Utf8String("hello"), 10)));
        Assert.That(result, Does.Contain(new Pair<Utf8String, int>(new Utf8String("bye"), 7)));
        Assert.That(result, Does.Contain(new Pair<Utf8String, int>(new Utf8String("foo"), 1)));
        Assert.That(result, Does.Contain(new Pair<Utf8String, int>(new Utf8String("hello"), 10)));
        Assert.That(result, Has.No.Member(new Pair<Utf8String, int>(new Utf8String("hello"), 9)));
        Assert.That(result, Has.No.Member(new Pair<Utf8String, int>(new Utf8String("bar"), 1)));
    }
