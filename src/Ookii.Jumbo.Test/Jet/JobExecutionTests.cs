...
public class JobExecutionTests : JobExecutionTestsBase
{
    ...
    public void TestJobAbort()
    {
        ...
        Assert.That(target.JobServer.GetJobStatus(job.JobId).IsSuccessful, Is.False);
        Thread.Sleep(5000);
    }
    ...
    public void TestSpillSortDynamicPartitionAssignment()
    {
        ...
        Assert.That(stage.Metrics.DynamicallyAssignedPartitions, Is.GreaterThan(0));
        Assert.That(stage.Metrics.DiscardedPartitions, Is.GreaterThan(0));
    }
    ...
    public void TestTaskTimeout()
    {
        ...
        Assert.That(status.Stages.Where(s => s.StageId == "WordCount").Single().Tasks[0].Attempts, Is.EqualTo(2));
    ...
    public void TestMultipleSimultaneousJobs()
    {
        ...
        Assert.That(status.ErrorTaskCount, Is.EqualTo(0));
